...
import { buildPrompt, streamChatCompletion } from '../utils/deepseek';
import { matchClassics, formatClassicsForAI } from '../data/classics';

/**
 * AI解读组件
 * 集成DeepSeek API进行八字解读
 */
export function AIAnalysis({ baziData }) {
  ...
  const startAnalysis = async () => {
    ...
    await streamChatCompletion(
      ...
      (err) => {
        setError(err.message || '请求失败，请检查API Key');
        setLoading(false);
      }
    );
  };
